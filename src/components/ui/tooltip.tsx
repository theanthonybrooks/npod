...
>(({ className, sideOffset = 4, children, ...props }, ref) => (
  <TooltipPrimitive.Portal>
    <TooltipPrimitive.Content
      ...
      className={cn(
        "group bg-card text-foreground z-50 overflow-hidden rounded-md border-[1.5px] px-3 py-1.5 text-xs",
        "opacity-0 transition-opacity duration-200",
      ...
      <Tooltip open={disabled ? false : undefined}>
        ...
        {!disabled && content ? (
          <TooltipContent
            ...
            className={cn(
              "group border-foreground z-50 overflow-hidden rounded-md border-[1.5px] bg-white px-3 py-1.5 text-xs text-black",
              "opacity-0 transition-opacity duration-200",
              "data-[state=closed]:opacity-0 data-[state=delayed-open]:opacity-100",
              "pointer-events-none",
              className,
